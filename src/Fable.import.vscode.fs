// ts2fable 0.6.1
module rec Fable.Import
open System
open Fable.Core
open Fable.Import.JS

let [<Import("*","module")>] vscode: Vscode.IExports = jsNative

module Vscode =
    let [<Import("commands","module/vscode")>] commands: Commands.IExports = jsNative
    let [<Import("debug","module/vscode")>] debug: Debug.IExports = jsNative
    let [<Import("env","module/vscode")>] env: Env.IExports = jsNative
    let [<Import("extensions","module/vscode")>] extensions: Extensions.IExports = jsNative
    let [<Import("languages","module/vscode")>] languages: Languages.IExports = jsNative
    let [<Import("scm","module/vscode")>] scm: Scm.IExports = jsNative
    let [<Import("tasks","module/vscode")>] tasks: Tasks.IExports = jsNative
    let [<Import("window","module/vscode")>] window: Window.IExports = jsNative
    let [<Import("workspace","module/vscode")>] workspace: Workspace.IExports = jsNative
    ...
    type [<AllowNullLiteral>] ThemeIconStatic =
        [<Emit "new $0($1...)">] abstract Create: id: string -> ThemeIcon
...
//        abstract pattern: GlobPattern option with get, set
    ...
    //     let ofDocumentFilter v: DocumentSelector = v |> U3.Case1
    //     let isDocumentFilter (v: DocumentSelector) = match v with U3.Case1 _ -> true | _ -> false
    //     let asDocumentFilter (v: DocumentSelector) = match v with U3.Case1 o -> Some o | _ -> None
    //     let ofString v: DocumentSelector = v |> U3.Case2
    //     let isString (v: DocumentSelector) = match v with U3.Case2 _ -> true | _ -> false
    //     let asString (v: DocumentSelector) = match v with U3.Case2 o -> Some o | _ -> None
    //     let ofArray v: DocumentSelector = v |> U3.Case3
    //     let isArray (v: DocumentSelector) = match v with U3.Case3 _ -> true | _ -> false
    //     let asArray (v: DocumentSelector) = match v with U3.Case3 o -> Some o | _ -> None
    ...
    //     let isThenable (v: ProviderResult<'T>) :bool= match v with None -> false | Some o -> match o with U2.Case2 _ -> true | _ -> false
    //     let asThenable (v: ProviderResult<'T>) :bool= match v with None -> None | Some o -> match o with U2.Case2 o -> Some o | _ -> None
    ...
    type [<AllowNullLiteral>] CodeActionKind =
        ...
        /// <summary>Does this kind contain `other`?
        /// 
        /// The kind `"refactor"` for example contains `"refactor.extract"` and ``"refactor.extract.function"`, but not `"unicorn.refactor.extract"` or `"refactory.extract"`</summary>
    ...
    type [<AllowNullLiteral>] CodeActionKindStatic =
        [<Emit "new $0($1...)">] abstract Create: value: string -> CodeActionKind
    ...
    [<RequireQualifiedAccess; CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
    module Declaration =
        let ofLocation v: Declaration = v |> U3.Case1
        let isLocation (v: Declaration) = match v with U3.Case1 _ -> true | _ -> false
        let asLocation (v: Declaration) = match v with U3.Case1 o -> Some o | _ -> None
        let ofLocationArray v: Declaration = v |> U3.Case2
        let isLocationArray (v: Declaration) = match v with U3.Case2 _ -> true | _ -> false
        let asLocationArray (v: Declaration) = match v with U3.Case2 o -> Some o | _ -> None
        let ofLocationLinkArray v: Declaration = v |> U3.Case3
        let isLocationLinkArray (v: Declaration) = match v with U3.Case3 _ -> true | _ -> false
        let asLocationLinkArray (v: Declaration) = match v with U3.Case3 o -> Some o | _ -> None
    ...
    type [<AllowNullLiteral>] RenameProvider =
        ...
        /// <param name="token">A cancellation token.</param>
        abstract prepareRename: document: TextDocument * position: Position * token: CancellationToken -> ProviderResult<U2<Range, obj>>
    ...
    type [<AllowNullLiteral>] ShellQuotingOptions =
        ...
        abstract escape: U2<string, obj> option with get, set
    ...
    type [<AllowNullLiteral>] FileSystemProvider =
        ...
        /// <param name="options">Defines if existing files should be overwriten.</param>
    ...
    type [<AllowNullLiteral>] WebviewPanelSerializer =
        /// <summary>Restore a webview panel from its seriailzed `state`.
        /// 
    ...
    module Window =
        ...
        type [<AllowNullLiteral>] IExports =
            ...
            abstract createWebviewPanel: viewType: string * title: string * showOptions: U2<ViewColumn, obj> * ?options: obj -> WebviewPanel
            ...
            abstract withProgress: options: ProgressOptions * task: (Progress<obj> -> CancellationToken -> Thenable<'R>) -> Thenable<'R>
    ...
    type [<AllowNullLiteral>] TreeItem =
        ...
        abstract iconPath: U4<string, Uri, obj, ThemeIcon> option with get, set
        /// A human readable string which is rendered less prominent.
    ...
    type [<AllowNullLiteral>] QuickInputButton =
        /// Icon for the button.
        abstract iconPath: U3<Uri, obj, ThemeIcon>
        /// An optional tooltip.
    ...
    type [<AllowNullLiteral>] QuickInputButtonsStatic =
        [<Emit "new $0($1...)">] abstract Create: unit -> QuickInputButtons
    ...
    module Workspace =
        ...
        type [<AllowNullLiteral>] IExports =
            ...
            abstract updateWorkspaceFolders: start: float * deleteCount: float option * [<ParamArray>] workspaceFoldersToAdd: ResizeArray<obj> -> bool
            ...
            abstract findFiles: ``include``: GlobPattern * ?exclude: GlobPattern option * ?maxResults: float * ?token: CancellationToken -> Thenable<ResizeArray<Uri>>
            ...
            abstract getConfiguration: ?section: string * ?resource: Uri option -> WorkspaceConfiguration
    ...
    module DebugAdapterDescriptor =
        let ofDebugAdapterExecutable v: DebugAdapterDescriptor = v |> U2.Case1
        let isDebugAdapterExecutable (v: DebugAdapterDescriptor) = match v with U2.Case1 _ -> true | _ -> false
        let asDebugAdapterExecutable (v: DebugAdapterDescriptor) = match v with U2.Case1 o -> Some o | _ -> None
        let ofDebugAdapterServer v: DebugAdapterDescriptor = v |> U2.Case2
        let isDebugAdapterServer (v: DebugAdapterDescriptor) = match v with U2.Case2 _ -> true | _ -> false
        let asDebugAdapterServer (v: DebugAdapterDescriptor) = match v with U2.Case2 o -> Some o | _ -> None
